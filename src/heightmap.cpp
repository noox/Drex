...
//	imageloader_load_heightmap (h, size_x, size_y);
	...
	for (i = 0;i < size_x;++i) {
		...
		for (j = 0;j < size_y;++j) {
			...
			vect A = vect(0, (D - C), tileheight * 
				(h[i+D*size_x] - h[i+C*size_x]));
			vect B = vect((b - a), 0, tileheight * 
				(h[b+j*size_x] - h[a+j*size_x]));
...
float heightmap::get_height(float x, float y)
{
	...
	if (my < 0.001) 
		return tileheight * tilesize * (mx * h2 + (1 - mx) * h1);
	float mxy = my / mx;
	return tileheight * tilesize * (mx * (h3 * mxy + h2 * (1 - mxy)) + 
		(1 - mx) * h1);
}
