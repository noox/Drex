...
{
	if (usernames.size() > 0)
		return usernames[userid].substr(0,
			usernames[userid].length() - 4);
	else return "";
}
...
void userlist_init()
{
	...
	while (1) {
		dp = readdir(dirp);
		if (dp) process_userfilename(dp->d_name);
		else break;
	}
	closedir(dirp);
}
...
string get_random_name(int t)
{
	...
	for (int i = 0;i < usernames.size();++i)
		...
		if (n.compare(userlist_get_name(i)) == 0)
			return get_random_name(t + 1);
